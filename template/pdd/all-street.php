			...
			foreach($allQuestions as $question) {
				...
				<div class="paginator">
					...
					<?php
						...
						if ($page != 1) $pervpage = '<a href= ./?city='.$_GET['city'].'&page=1>Первая</a>  
								   <a href= ./?city='.$_GET['city'].'page='. ($page - 1) .'><</a> ';  
						else {$pervpage = null;}
						// Проверяем нужны ли стрелки вперед  
						if ($page != $total) $nextpage = ' <a href= ./?city='.$_GET['city'].'&page='. ($page + 1) .'>></a>  
														   <a href= ./?city='.$_GET['city'].'&page=' .$total. '>Последняя</a>';  
						else {$nextpage = null;}

						// Находим две ближайшие станицы с обоих краев, если они есть  
						if($page - 4 > 0) $page4left = ' <a href= ./?city='.$_GET['city'].'&page='. ($page - 4) .'>'. ($page - 4) .'</a>';  
						else {$page4left = null;}
						
						if($page - 3 > 0) $page3left = ' <a href= ./?city='.$_GET['city'].'&page='. ($page - 3) .'>'. ($page - 3) .'</a>';  
						else {$page3left = null;}
						
						if($page - 2 > 0) $page2left = ' <a href= ./?city='.$_GET['city'].'&page='. ($page - 2) .'>'. ($page - 2) .'</a>';  
						else {$page2left = null;}
						
						if($page - 1 > 0) $page1left = '<a href= ./?city='.$_GET['city'].'&page='. ($page - 1) .'>'. ($page - 1) .'</a>';  
						else {$page1left = null;}
						
						
						if($page + 4 <= $total) $page4right = '<a href= ./?city='.$_GET['city'].'&page='. ($page + 4) .'>'. ($page + 4) .'</a>'; 
						else {$page4right = null;}
						
						if($page + 3 <= $total) $page3right = '<a href= ./?city='.$_GET['city'].'&page='. ($page + 3) .'>'. ($page + 3) .'</a>'; 
						else {$page3right = null;}
						
						if($page + 2 <= $total) $page2right = '<a href= ./?city='.$_GET['city'].'&page='. ($page + 2) .'>'. ($page + 2) .'</a>'; 
						else {$page2right = null;}
						
						if($page + 1 <= $total) $page1right = '<a href= ./?city='.$_GET['city'].'&page='. ($page + 1) .'>'. ($page + 1) .'</a>'; 
